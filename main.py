...
class System:
    ...
    def system_info_interface(self):
        ...
        def processes_info():
            from tabs import processes

            processes_use = processes.get_running_processes()

            # Create a frame to contain the process information
            process_frame = Frame(processes_tab)
            process_frame.pack(fill=BOTH, expand=YES)

            # Create a Treeview widget to display the process information
            process_tree = ttk.Treeview(process_frame, columns=("pid", "name", "status", "cpu_percent", "memory_info", "create_time", "username"))
            ...
            process_tree.pack(fill=BOTH, expand=YES)
